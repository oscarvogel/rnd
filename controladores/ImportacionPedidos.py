...
class ImportacionPedidosController(ControladorBase):
    ...
    def on_click_btn_grabar(self, *args, **kwargs):
        """Graba pedidos con reintento de SELECTs y sin repetir escrituras inciertas."""
        ...
        try:
            ...
            if not columna_cliente:
                self.view.avance.marcar_error("Configuración incompleta")
                showAlert(
                    "Sistema",
                    "No está configurada la columna Cliente para este proveedor.",
                )
                return
        ...
        for row in range(total):
            ...
            nombre_cliente = self.view.grid_datos.ObtenerItem(
                fila=row,
                col=columnas.get("Nombre_Cliente"),
            )
            ...
            try:
                ...
                if busqueda:
                    ...
                    self._leer_db_con_reintento(
                        lambda: buscador_cliente.buscar(self.view),
                        "búsqueda interactiva de cliente",
                    )
                    if buscador_cliente.lRetval:
                        # get_or_create puede escribir: no se reintenta automáticamente.
                ...
                if observaciones:
                    hoja_ruta.observaciones = self.view.grid_datos.ObtenerItem(
